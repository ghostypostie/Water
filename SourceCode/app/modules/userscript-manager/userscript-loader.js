	...
	load() {
		try {
			...
			if (this.settings && Object.keys(this.settings).length > 0 && existsSync(this.settingsPath)) {
				try {
					const settingsJSON = JSON.parse(readFileSync(this.settingsPath, 'utf-8'));
					Object.keys(settingsJSON).forEach(settingKey => {
						if (settingKey in this.settings
							...
							&& settingsJSON[settingKey] !== this.settings[settingKey].value
							&& typeof settingsJSON[settingKey] === typeof this.settings[settingKey].value) {
 ...
 * @param {string} userscriptPrefsPath - Path to userscript preferences directory
 * @param {object} config - Electron-store config instance
 */
function initializeUserscripts(userscriptsPath, userscriptPrefsPath, config) {
	su.userscriptsPath = userscriptsPath;
	su.userscriptPrefsPath = userscriptPrefsPath;
	su.config = config;
	...
	su.userscripts = readdirSync(su.userscriptsPath, { withFileTypes: true })
		...
		.map(entry => new Userscript({
			name: entry.name,
			settingsPath: pathResolve(su.userscriptPrefsPath, entry.name.replace(/.js$/u, '.json')),
