...
class WindowManager {
	...
	constructor(callerId, hideKrunkerWindowsOnShow = true) {
		...
		document.addEventListener("DOMContentLoaded", () => {
			if (!document.getElementById("Water-windowHolder")) {
				let w = document.createElement("div");
				w.setAttribute("id", "Water-windowHolder");
				w.setAttribute("style", "display: none;");
				w.innerHTML = '<div id="Water-menuWindow"></div>';
				...
				document.getElementsByTagName("body")[0].addEventListener("click", e => {
					// @ts-ignore
					(!e.path.find(p => p.id === "Water-menuWindow" || p.id === this.callerId)) && this.hide();
				});
			}
		});
	}
	 ...
	 */
	setContent(content) {
		document.getElementById("Water-menuWindow").innerHTML = content;
	}
	...
	show() {
		...
		document.getElementById("Water-windowHolder").setAttribute("style", "display: block;");
		this.shown = true;
	}
	 ...
	 */
	hide() {
		document.getElementById("Water-windowHolder").setAttribute("style", "display: none;");
		this.shown = false;
	}
