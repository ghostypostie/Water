...
class Chat extends module_1.default {
    ...
    showFilterNotification(categoryKey) {
    ...
    categorizeMessage(elem) {
        let msgNode = elem.firstChild;
        if (msgNode.childNodes.length > 1)
            elem.classList.add('msg--player');
        else {
            if (msgNode.firstChild.childNodes.length > 1) {
                if (msgNode.firstChild.children.length > 1 &&
                    ![...msgNode.firstChild.childNodes].some((e) => e.nodeName === 'IMG'))
                    elem.classList.add('msg--unbox');
                else
                    elem.classList.add('msg--killfeed');
            }
            else
    ...
    renderer() {
        ...
        window.chatList = Object.defineProperties({}, {
            childNodes: {
                get: () => [...elem.childNodes].filter((e) => e.classList?.contains('vanillaChatMsg')),
            },
            removeChild: {
            ...
            insertAdjacentElement: {
                value: (position, element) => {
                    ...
                    mock.insertAdjacentElement(position, document.createElement('div'));
