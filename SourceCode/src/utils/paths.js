...
exports.getWaterPath = getWaterPath;
exports.getSwapPath = getSwapPath;
exports.getScriptsPath = getScriptsPath;
 ...
 */
function getWaterPath() {
    const possiblePaths = [];
    const userProfile = process.env.USERPROFILE || process.env.HOME || '';
    if (userProfile) {
        // Method 1: Check OneDrive Documents folder
        const oneDriveDocs = (0, path_1.join)(userProfile, 'OneDrive', 'Documents', 'Water');
        possiblePaths.push(oneDriveDocs);
        // Method 2: Check local Documents folder
        const localDocs = (0, path_1.join)(userProfile, 'Documents', 'Water');
        possiblePaths.push(localDocs);
    }
    // Find all existing paths
    const existingPaths = possiblePaths.filter(p => (0, fs_1.existsSync)(p));
    if (existingPaths.length === 0) {
        // No existing folder, use local Documents as default
        const defaultPath = (0, path_1.join)(userProfile, 'Documents', 'Water');
    ...
    const pathsWithCounts = existingPaths.map(path => {
        const swapFiles = countFiles((0, path_1.join)(path, 'Swap'));
        const scriptFiles = countFiles((0, path_1.join)(path, 'Scripts'));
        const cssFiles = countFiles((0, path_1.join)(path, 'Swap', 'css'));
...
function getSwapPath() {
    return (0, path_1.join)(getWaterPath(), 'Swap');
}
/**
 * Get the Scripts folder path
 */
function getScriptsPath() {
    return (0, path_1.join)(getWaterPath(), 'Scripts');
}
